...
export default function App() {
  ...
  const handleEnrich = async (enrichBody, isAuto = false) => {
    ...
    // FilterPanel always sends page:1 — override with the actual current page
    // so the backend slices the same rows that are visible on screen.
    const body = {
      ...enrichBody,
      page:      lastFilter?.page      ?? enrichBody.page,
      page_size: lastFilter?.page_size ?? enrichBody.page_size,
    }

    try {
      const data = await runEnrich(body)

      // Normalise contentid to plain string (strips trailing ".0" from float ids)
      const normalise = (id) => String(id ?? '').replace(/\.0$/, '')

      _setFilterResult(prev => {
        if (!prev || !data.results) return prev
        const enrichedContents = prev.contents.map(item => {
          const matchData = data.results.find(
            res => normalise(res.contentid) === normalise(item.contentid)
          )
  ...
  return (
    <div className={`content-card${highlight ? ' content-card--highlight' : ''}`} style={{ position: 'relative' }}>
      ...
      {advResults && (
        <div className="carousel-section" style={{ padding: '16px 0 0 0', marginTop: '16px', borderTop: '1px solid var(--border)' }}>
          <div className="carousel-label">
            🔍 Advanced Search Results ({advResults.length})
            <button className="carousel-close-btn" onClick={() => { setAdvResults(null); setAdvSelectedId(null); }} title="Hide">✕</button>
          </div>
          {advResults.length === 0 ? (
      ...
      {dubbedResults && (
        <div className="carousel-section" style={{ padding: '16px 0 0 0', marginTop: '16px', borderTop: '1px solid var(--border)' }}>
          <div className="carousel-label">
            🎙️ Dubbed Matches ({dubbedResults.length})
            <button className="carousel-close-btn" onClick={() => { setDubbedResults(null); setDubbedSelectedId(null); }} title="Hide">✕</button>
          </div>
